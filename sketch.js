...
function preload() {
  ...
  // Story Videos
  videos["api"] = createVideo("api.mp4");
  videos["intro1"] = createVideo("INTRODUCTION 1.mp4");
  videos["intro2"] = createVideo("INTRODUCTION 2.mp4");
  videos["q1"] = createVideo("QUESTION 1.mp4");
  videos["q2"] = createVideo("QUESTION 2.mp4");
  videos["q3"] = createVideo("QUESTION 3.mp4");
  videos["q4"] = createVideo("QUESTION 4.mp4");
  videos["q5"] = createVideo("QUESTION 5.mp4");
  videos["q6"] = createVideo("QUESTION 5.mp4"); // same file as q5
  videos["q7"] = createVideo("QUESTION 7.mp4");
  videos["q8"] = createVideo("QUESTION 8.mp4");

  // Result Videos
  videos["mage_res"] = createVideo("RESULTMAGE.mp4");
  videos["tank_res"] = createVideo("RESULTTANK.mp4");
  videos["rogue_res"] = createVideo("RESULTROGUE.mp4");
  videos["support_res"] = createVideo("RESULTSUPPORT.mp4");
...
function setup() {
  ...
  portalEnterBtn.position(
    canvasOffsetX + (1100 / 2 - 230),
    canvasOffsetY + 700
  );
  ...
  walkAwayBtn.mousePressed(() => {
    ...
    portalEnterBtn.style(
      "box-shadow",
      `0 0 ${glow}px #f0f, inset 0 0 10px #f0f`
    );
  });

  // --- REFLECTION INPUT ---
  reflectionInput = createElement("textarea", "").hide();
  reflectionInput.position(
    canvasOffsetX + (1100 / 2 - 250),
    canvasOffsetY + 580
  );
  ...
  reflectionInput.attribute(
    "placeholder",
    "PLEASE SHARE YOUR NAME AND YOUR STORY..."
  );
  ...
  if (optionButtons.length === 0) {
    ...
    for (let i = 0; i < options.length; i++) {
      let btn = createButton(options[i].txt)
        ...
        .position(
          canvasOffsetX + startX + i * (btnW + gap),
          canvasOffsetY + posY
        )
        .size(btnW, btnH);
      ...
      btn.mousePressed(() => {
        ...
        userAnswers.push(
          `Choice: ${options[i].txt} (Trait: ${options[i].trait})`
        );
...
function draw() {
  ...
  if (!isHoveringClickHere) {
    ...
    let overInfo =
      mouseX - canvasOffsetX > infoX &&
      mouseX - canvasOffsetX < infoX + infoW &&
      mouseY - canvasOffsetY > infoY &&
      mouseY - canvasOffsetY < infoY + infoH;
    ...
    textSize(10);
    text(
      "<  R E V E A L   G A M E   M A N U A L  >",
      infoX + infoW / 2,
      infoY + infoH / 2
    );
    pop();
...
function advance() {
  ...
  if (currentStep < storyFlow.length) {
    ...
    if (
      storyFlow[currentStep].type === "openQ" &&
      currentStep === storyFlow.length - 1
    ) {
...
async function generatePersonalizedQuestion() {
  ...
  try {
    ...
    storyFlow[currentStep].story =
      "What did you discover about yourself in the mist?";
  }
}
...
async function finalizeQuiz() {
  ...
  let classTheme = "";
  if (topClass === "tank")
    classTheme = "Protection, Resilience, and Unwavering Fortitude";
  else if (topClass === "mage")
    classTheme = "Arcane Wisdom, Strategic Insight, and Mastery";
  else if (topClass === "rogue")
    classTheme = "Precision, Agility, and Calculated Risk";
  else if (topClass === "support")
    classTheme = "Synergy, Restoration, and Empowering Others";
  ...
  try {
    ...
    aiMotto = mottoLine
      ? mottoLine.replace(/MOTTO:/i, "").trim()
...
function drawResult() {
  ...
  if (isHoveringClickHere && handImg) {
    ...
    stats.forEach((s, i) => {
      ...
      else if (s.label === "INTELLECT") barColor = color(50, 200, 255);
      // Neon Blue
...
const storyFlow = [
...

  {
    ...
    options: [
      ...
      {
        txt: "B) Keep your distance and strategize before engaging.",
        trait: "mage",
      },
...
function endMiniGame(reason) {
  userAnswers.push(
    `Battle Performance: ${orcsKilled} orcs defeated. Reason: ${reason}`
  );
